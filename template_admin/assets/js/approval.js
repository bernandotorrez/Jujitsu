...
function search() {
 ...
 var data_ref_code = $("#ref_code").val();
 var data_csrf_protection = $("#csrf_protection").val();
 $.ajax({

  type: "POST", // Method pengiriman data bisa dengan GET atau POST
  url: "http://localhost/jujitsu/Admin/cari_approval", // Isi dengan url/path file php yang dituju
  data: 
   
   { ref_code: data_ref_code, csrf_protection: data_csrf_protection },
  ...
  statusCode: {
    403: function() {
       ...
       $("#data").html('Session Anda Habis, Silahkan Reload Halaman Ini');
       $("#nodata").html('');
  ...
  success: function(response) { // Ketika proses pengiriman berhasil
   ...
   if (response.status == "success") { // Jika isi dari array status adalah success
    ...
    $("#nim").val(response.nim).prop('disabled', true);          
    $("#email").val(response.email).prop('disabled', true);
    $("#nama_jujitsan").val(response.nama).prop('disabled', true);

    var jk = response.jenis_kelamin;
  
    if(jk == 'L'){
       $("#jenis_kelamin").val('Laki - Laki').prop('disabled', true);
    } else if(jk == 'P'){
        $("#jenis_kelamin").val('Perempuan').prop('disabled', true);
    }

    $("#no_hp").val(response.no_hp).prop('disabled', true);
    $("#whatsapp").val(response.whatsapp).prop('disabled', true);
    $("#line").val(response.line).prop('disabled', true);
    $("#ref_code1").val(response.ref_code).prop('disabled', true);
    $("#tempat_lahir").val(response.tempat_lahir).prop('disabled', true);
    $("#tanggal_lahir").val(response.tanggal_lahir).prop('disabled', true);
    $("#fakultas").val(response.fakultas).prop('disabled', true);
    $("#jurusan").val(response.jurusan).prop('disabled', true);
    $("#angkatan_kuliah").val(response.angkatan_kuliah).prop('disabled', true);
